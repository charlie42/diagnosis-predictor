...
def make_performance_table(performances_on_subsets, number_of_features_to_check):
    performances_on_subsets = pd.DataFrame.from_dict(performances_on_subsets)
    performances_on_subsets.index = range(1, number_of_features_to_check+1)
...
def make_auc_table(performance_table):
    ...
    roc_table[new_columns].to_csv(reports_dir+'auc_on_subsets.csv')
...
def make_sens_spec_table(performance_table):
    ...
    # Sort columns by score on 100 features (first row)
    new_columns = get_diags_in_order_of_auc_on_max_features(performance_table)
    sens_spec_table[new_columns].to_csv(reports_dir+'sens_spec_on_subsets.csv')

def get_top_n_feaures(n, importances, diag):
    features_up_top_n = importances[diag][:n]
    return features_up_top_n

def re_train_models_on_feature_subsets_per_output(diag, number_of_features_to_check, importances, datasets, best_classifiers):
    ...
    for nb_features in range(1, number_of_features_to_check+1):
        ...
        top_n_features = get_top_n_feaures(nb_features, importances, diag)
...
def re_train_models_on_feature_subsets(importances, datasets, best_classifiers, number_of_features_to_check):
    classifiers_on_feature_subsets = {}
    for diag in importances.keys():
        print("Re-training models on feature subsets for output: " + diag)
        classifiers_on_feature_subsets[diag] = re_train_models_on_feature_subsets_per_output(diag, number_of_features_to_check, importances, datasets, best_classifiers)
        
    return classifiers_on_feature_subsets

def calculate_thresholds_for_feature_subsets_per_output(diag, importances, classifiers_on_feature_subsets, datasets, number_of_features_to_check):
    X_train, y_train = datasets[diag]["X_train_train"], datasets[diag]["y_train_train"]
...
def calculate_thresholds_for_feature_subsets(importances, classifiers_on_feature_subsets, datasets, number_of_features_to_check):
    thresholds_on_feature_subsets = {}
    for diag in importances.keys():
        thresholds_on_feature_subsets[diag] = calculate_thresholds_for_feature_subsets_per_output(diag, importances, classifiers_on_feature_subsets, datasets, number_of_features_to_check)
    return thresholds_on_feature_subsets

def get_performances_on_feature_subsets_per_output(diag, importances, classifiers_on_feature_subsets, thresholds_on_feature_subsets, datasets, number_of_features_to_check, use_test_set):
...
def main(number_of_features_to_check = 100, models_from_file = 1):
    ...
    features_in_importance_order = load(models_dir+'features-in-importance-order.joblib')
    datasets = load(models_dir+'datasets.joblib')
    best_classifiers = load(models_dir+'best-classifiers.joblib')

    if models_from_file == 1:
        performances_on_feature_subsets = load(models_dir+'performances-on-feature-subsets.joblib')    
    else:
        performances_on_feature_subsets = get_performances_on_feature_subsets(features_in_importance_order, datasets, best_classifiers, number_of_features_to_check, use_test_set = 1)
        dump(performances_on_feature_subsets, models_dir+'performances-on-feature-subsets.joblib')

    performance_table = make_performance_table(performances_on_feature_subsets, number_of_features_to_check)
    print(performance_table)
    performance_table.to_csv(reports_dir+'performances-on-feature-subsets.csv')
